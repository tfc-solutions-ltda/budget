...
export default function NewBudgetPage() {
  ...
  const totalHoursPerStory = stories.map(story => ({
    id: story.id,
    baseHours: story.activities.reduce((sum, activity) => sum + activity.hours, 0),
    hoursWithComplexity: story.activities.reduce((sum, activity) => 
      sum + calculateHoursWithComplexity(activity.hours, activity.complexityFactor, story.complexityFactor), 0)
  }));
  ...
  const totalHoursWithProjectComplexity = totalHoursWithAllComplexity * projectComplexityFactor;
  const testHours = (totalHoursWithProjectComplexity * testPercentage) / 100;
  ...
  const updateActivity = (storyId: string, activityId: string, field: keyof Activity, value: string | number) => {
    setStories(stories.map(story => 
      story.id === storyId 
        ? {
            ...story,
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-col space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-base font-medium text-white">Configurações</h3>
                      <button
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex justify-end space-x-4 mt-8">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                >
                  {loading ? (
                    <>
